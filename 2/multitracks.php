...
<head>
    ...
    <title>Mixer - <?php echo isset($_GET['song']) ? htmlspecialchars(urldecode($_GET['song'])) : 'Música'; ?></title>
    <style>
        :root { --primary: #6366f1; --secondary: #8b5cf6; --dark: #1e293b; --success: #10b981; --danger: #ef4444; }
        body { font-family: 'Segoe UI', sans-serif; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px; min-height: 100vh; }
        .container { max-width: 1000px; margin: 0 auto; background: rgba(255, 255, 255, 0.98); padding: 30px; border-radius: 20px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); }
        ...
        .btn-back { background: linear-gradient(135deg, var(--primary), var(--secondary)); color: white; padding: 12px 25px; border-radius: 50px; text-decoration: none; font-weight: bold; }
        .master-controls { background: var(--dark); color: white; padding: 25px; border-radius: 15px; margin-bottom: 30px; text-align: center; position: sticky; top: 10px; z-index: 100; }
        ...
        .progress-container { margin: 20px 0; display: flex; align-items: center; gap: 10px; background: rgba(255,255,255,0.1); padding: 15px; border-radius: 10px; }
        .progress-container input[type="range"] { flex-grow: 1; cursor: pointer; height: 8px; border-radius: 5px; }
        .time-display { font-family: monospace; font-size: 14px; min-width: 45px; }
        ...
        audio { height: 35px; width: 100%; display: none; } /* Escondido pois usamos os controles mestres */
    </style>
</head>
<body>
...
<div class="container">
    <div class="header-nav">
        ...
        <h1 style="font-size:1.5em;">🎵 <?php echo isset($_GET['song']) ? htmlspecialchars(urldecode($_GET['song'])) : 'Selecione'; ?></h1>
    ...
    <div class="master-controls">
        ...
        <div class="progress-container">
            ...
            <input type="range" id="mainProgress" value="0" min="0" step="0.1" oninput="seekAll(this.value)">
            <span class="time-display" id="duration">0:00</span>
        </div>
        
        <div style="background:rgba(255,255,255,0.1); padding:10px; border-radius:10px; display:inline-block;">
            Velocidade: <input type="range" id="speed" min="0.5" max="2" step="0.1" value="1" oninput="updateSpeed(this.value)">
    ...
    <div class="track-list">
        ...
        if (isset($_GET['song'])) {
            ...
            $baseDir = __DIR__ . '/instrumentos/' . $songName;
            ...
            if ($mp3Files) {
                foreach ($mp3Files as $file) {
                    ...
                    <div class="track-item">
                        ...
                        <div><audio class="track-audio" preload="auto" data-id="'. htmlspecialchars($cleanName) .'"><source src="'.$webPath.'" type="audio/mpeg"></audio></div>
                        <div><input type="range" min="0" max="1" step="0.01" value="1" oninput="updateVol(this, \''. htmlspecialchars($cleanName) .'\')"></div>
...
// Sincroniza a duração total baseada no primeiro áudio carregado
if(audios.length > 0) {
    audios[0].addEventListener('loadedmetadata', () => {
        progressBar.max = audios[0].duration;
        durationEl.textContent = formatTime(audios[0].duration);
    });
}
...
setInterval(() => {
    if (audios.length > 0 && !audios[0].paused) {
        progressBar.value = audios[0].currentTime;
        currentTimeEl.textContent = formatTime(audios[0].currentTime);
    }
}, 500);
...
function playAll() { audios.forEach(a => { a.playbackRate = currentSpeed; a.play(); }); }
function pauseAll() { audios.forEach(a => a.pause()); }
function updateSpeed(v) { currentSpeed = v; document.getElementById('speedValue').textContent = v + 'x'; audios.forEach(a => a.playbackRate = v); }
...
function toggleSolo(btn, id) {
    ...
    if (active) {
        ...
        audio.muted = false;
        document.querySelectorAll('.btn-solo').forEach(b => { if(b !== btn) b.classList.remove('active'); });
    } else {
